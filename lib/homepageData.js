...
export async function fetchHomepageData() {
  await dbConnect();
  // Ensure Location schema is registered for populate
  ...
  const [
    ...
    Animal.find({
      ...
      .select(
        "tagId name species breed gender currentWeight projectedSalesPrice salesPrice images location dob"
      )
    ...
    Animal.aggregate([
      ...
      },
      {
        $group: {
          _id: "$species",
          count: { $sum: 1 },
          avgPrice: { $avg: { $max: ["$salesPrice", "$projectedSalesPrice"] } },
          sample: { $first: "$images" },
        },
      },
      { $sort: { count: -1 } },
